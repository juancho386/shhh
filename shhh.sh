...
write_msg () {
	...
	if [[ "$resp" != "" ]]; then 
		...
		#test: to decrypt: openssl enc -d -chacha20 -pbkdf2 -pass "pass:${password}" <<< "${encriptedMsg}"
		local encryptedPassword=$(openssl rsautl -encrypt -pubin -inkey $pub_key_pem 2>/dev/null <<< "$password"|base64 -w0)
...
read_msg () {
	...
	if [[ -f shhh.private.pem ]]; then
		...
		password=$(base64 -d <<< $encodedPassword | openssl rsautl -decrypt -inkey shhh.private.pem 2>/dev/null )
